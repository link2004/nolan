...
## 参照PiP + カメラロール保存 + モーションコーチ (2026-07-05)

ADR-005。撮影画面の3機能追加。

- [x] ShotReference(URL解決済み参照)+ SBBoard マッピング拡張
- [x] 参照PiP(右上4:5、クリップはミュートループ、静止画はAsyncImage)
- [x] PhotoLibraryClient + OK承認時にカメラロール保存(失敗バナー)
- [x] MotionCoach: shot_direction キーワード検出(11モーション)+ trim描画アニメの矢印注釈
- [x] 両スキームビルド + CueCam 実機インストール・起動(前回保留分も解消)
- [ ] 実機確認(ユーザー): PiP再生 / モーション矢印の見え方 / 写真アプリへの保存
- [ ] モーション検出の精度チューニング(実ボードの言い回しを見ながら)
